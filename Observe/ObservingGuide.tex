...
\item In MaxIM CCD window, go to the \emph{Focus} tab. Click on the \emph{CCD} radio button. {\bf Make sure binning is set to 3 when CCD radio button is selected!} Set the exposure time between 1-5 seconds depending on quality of image. \emph{Delay} should be set at 0. Click \emph{Start Focus}. A window of the field of view will open. Click \emph{Stop}. The dimensions of this window should be the same as the dimensions of the CCD window: 3072 x 2048. Find the focus star in that image, right click on it and select \emph{point telescope here}.
